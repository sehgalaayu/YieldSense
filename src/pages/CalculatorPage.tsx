...
export default function CalculatorPage() {
  ...
  const nearestTenor = useMemo(() => {
    ...
    const tenors = Array.from(new Set(fdRates.map((p) => p.tenor))) as number[];
    tenors.sort((a, b) => a - b);
    return tenors.reduce((prev, curr) =>
      Math.abs(curr - calcInput.tenorMonths) <
      Math.abs(prev - calcInput.tenorMonths)
        ? curr
        : prev,
    );
  ...
  const results = useMemo(() => {
    if (fdRates.length === 0)
      return calculateYield({
        ...calcInput,
        grossRate: 7,
        tenorMonths: calcInput.tenorMonths,
      });
    ...
    return fdRates
      ...
      .map((p) => {
        const res = calculateYield({
          ...
          tenorMonths: nearestTenor,
        });
        return {
          name: p.bankName
            .replace(" Bank", "")
            .replace(" Ltd", "")
            .split(" ")[0], // Short bank name
          fullName: p.bankName,
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-12">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl font-semibold tracking-tight text-[#F1F5F9]">
            {activeTab === "FD"
              ? language === "hi" ? "FD यील्ड कैलकुलेटर" : "FD Yield Calculator"
              : language === "hi" ? "SIP रिटर्न कैलकुलेटर" : "SIP Returns Calculator"}
          </h1>
          <p className="text-[#64748B] text-sm mt-1">
            {activeTab === "FD"
              ? language === "hi" ? "TDS और इनकम टैक्स के बाद वास्तविक कमाई देखें" : "See what you actually earn after TDS and income tax"
              : language === "hi" ? "मासिक SIP निवेश से अपनी संपत्ति वृद्धि का अनुमान लगाएं" : "Project your wealth growth with monthly SIP investments"}
          </p>
        </div>
        <div className="flex bg-[#0A0F1E] rounded-xl border border-[#1E3A5F] p-1">
          <button
            onClick={() => setActiveTab("FD")}
            className={`px-6 py-2 text-sm font-medium rounded-lg transition-all ${activeTab === "FD" ? "bg-[#1E3A5F] text-white shadow-md" : "text-[#64748B] hover:text-white"}`}
          >
            Lump Sum FD
          </button>
          <button
            onClick={() => setActiveTab("SIP")}
            className={`px-6 py-2 text-sm font-medium rounded-lg transition-all ${activeTab === "SIP" ? "bg-[#1E3A5F] text-white shadow-md" : "text-[#64748B] hover:text-white"}`}
          >
            Mutual Fund SIP
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
          ...
          <div className="lg:col-span-5 space-y-8 bg-bg-secondary p-8 rounded-3xl border border-border-subtle h-fit">
            <div className="space-y-4">
              ...
              <div className="relative">
                <span className="absolute left-4 top-1/2 -translate-y-1/2 text-accent-blue font-bold">
                  ₹
                </span>
                <input
                  type="number"
            ...
            <div className="space-y-4">
              ...
              <div className="relative pt-2">
                <input
                  ...
                  max="6"
                  step="1"
                  value={[3, 6, 12, 18, 24, 36, 60].indexOf(
                    calcInput.tenorMonths,
                  )}
                  onChange={(e) => {
                    const val = [3, 6, 12, 18, 24, 36, 60][
              ...
              <div className="grid grid-cols-4 gap-2">
                {[0, 5, 20, 30].map((s) => (
                  <button
                    key={s}
                    onClick={() =>
                      setCalcInput({ ...calcInput, taxSlab: s as any })
                    }
            ...
            <div className="space-y-4">
              ...
              <div className="flex bg-bg-tertiary p-1 rounded-xl border border-border-subtle">
                <button
                  onClick={() =>
                    setCalcInput({ ...calcInput, interestType: "Cumulative" })
                  }
          ...
          <div className="lg:col-span-7 space-y-8">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="bg-bg-tertiary p-6 rounded-2xl border border-border-subtle">
                ...
                <div className="text-2xl font-mono font-bold">
                  {formatCurrency(results.grossMaturityAmount)}
                </div>
              </div>
              <div className="bg-bg-tertiary p-6 rounded-2xl border border-border-subtle">
                <div className="flex items-center gap-2 mb-2">
                ...
                <div className="text-2xl font-mono font-bold text-accent-red">
                  -{formatCurrency(results.taxPayable + results.tdsDeducted)}
                </div>
              </div>
            </div>
            ...
            <div className="bg-bg-secondary p-8 rounded-3xl border border-accent-gold/30 relative overflow-hidden">
              ...
              <label className="text-xs font-bold uppercase tracking-widest text-accent-gold mb-2 block">
                {t.results.item3}
              </label>
              ...
              <div className="flex items-center gap-4">
                <div className="flex items-center gap-1 bg-accent-green/20 text-accent-green px-3 py-1 rounded-full text-sm font-bold">
                  <Percent size={14} />
                  {results.effectiveAnnualYield.toFixed(2)}% p.a.{" "}
                  {t.results.yieldNotice}
                </div>
                {results.monthlyPayout && (
                  <div className="text-sm text-text-muted">
                    + {formatCurrency(results.monthlyPayout)} /{" "}
                    {t.results.payoutNotice}
                  </div>
                )}
              </div>
            </div>
                  ...
                  <BarChart data={chartData}>
                    ...
                    <Bar dataKey="yield" radius={[6, 6, 0, 0]}>
                      {chartData.map((_entry, index) => {
                        ...
                        const opacity = isHighest
                          ? 1
                          : 0.4 + (index / chartData.length) * 0.6;
                        return (
                          <Cell
                            key={`cell-${index}`}
                            fill={fillColor}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
            ...
            <div className="space-y-4">
              ...
              <div className="relative pt-2">
                <input
                  ...
                  onChange={(e) =>
                    setSipInput({ ...sipInput, years: Number(e.target.value) })
                  }
                  className="w-full h-2 bg-bg-tertiary rounded-lg appearance-none cursor-pointer accent-accent-blue"
                />
              </div>
          ...
          <div className="lg:col-span-7 space-y-8">
            ...
            <div className="bg-bg-secondary p-8 rounded-3xl border border-accent-gold/30 relative overflow-hidden">
              ...
              <label className="text-xs font-bold uppercase tracking-widest text-accent-gold mb-2 block">
                Net Maturity Value (after LTCG tax)
              </label>
              <div className="text-6xl font-mono font-extrabold mb-4">
                {formatCurrency(sipResults.netMaturityValue)}
              </div>
            ...
            <div className="bg-[#112240] rounded-2xl p-4 border border-[#1E3A5F] flex items-center gap-3">
              ...
              <div>
                <p className="text-sm text-[#F1F5F9]">
                  In today's money (inflation-adjusted at{" "}
                  {sipInput.inflationPct}%):
            ...
            <div className="bg-bg-secondary p-8 rounded-3xl border border-border-subtle">
                  ...
                  <AreaChart data={sipResults.monthlyData}>
                    ...
                    <Tooltip
                      ...
                      labelFormatter={(label) =>
                        `Month ${label} (Year ${Math.floor(label / 12)})`
                      }
                    />
                    <Area
                      type="monotone"
              ...
              <p className="text-[10px] text-text-muted text-center mt-4">
                * Calculations assume equity mutual fund taxation (12.5% LTCG on
                gains &gt; ₹1.25L).
              </p>
            </div>
          </div>
