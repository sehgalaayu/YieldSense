...
import { ShieldCheck, TrendingUp, Clock, Coins, AlertTriangle, Info } from 'lucide-react';
...
export default function CompareInstrumentsPage() {
  ...
  const fdTDS = fdInterest > 40000 ? fdInterest * 0.10 : 0;
  ...
  const chartData = useMemo(() =>
    Array.from({ length: years }, (_, i) => {
      ...
      const mfN = mfG - mfT;
      return { year: `Y${yr}`, FD: Math.round(fdN), 'Mutual Fund': Math.round(mfN) };
  ...
  const insights = [
    ...
    },
    {
      ...
      title: language === 'hi' ? 'MF लंबे समय में ज़्यादा बढ़ते हैं' : 'MF compounds harder over time',
      body: language === 'hi'
    ...
    },
    {
      ...
      title: language === 'hi' ? 'टैक्स ट्रीटमेंट अलग है' : 'Tax treatment differs',
      body: language === 'hi'
    ...
    },
    {
      ...
      title: language === 'hi' ? 'समय सबसे ज़्यादा मायने रखता है' : 'Time horizon matters most',
      body: years < 3
        ? (language === 'hi'
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-7xl mx-auto px-4 sm:px-6 py-8 sm:py-12">
      ...
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-10">
        ...
        <h1 className="text-3xl sm:text-4xl lg:text-5xl font-syne font-bold mb-4 text-white" style={{ textWrap: 'balance' as any }}>
          {language === 'hi' ? 'FD या Mutual Fund — आपके लिए क्या बेहतर है?' : 'Should you put your money in an FD or a Mutual Fund?'}
        </h1>
      ...
      >
        ...
        <div>
          <label className="text-xs font-bold uppercase tracking-widest text-[#64748B] mb-2 block">
            {language === 'hi' ? 'निवेश अवधि (वर्ष)' : 'Investment Horizon (Years)'}
          </label>
          <input type="range" min={1} max={20} value={years} onChange={e => setYears(Number(e.target.value))}
            className="w-full accent-[#1A56DB] mt-2" />
          <div className="text-center text-white font-mono font-bold text-lg mt-1">{years} {language === 'hi' ? 'वर्ष' : 'years'}</div>
        </div>
        <div>
          <label className="text-xs font-bold uppercase tracking-widest text-[#64748B] mb-2 block">
        ...
        <div>
          <label className="text-xs font-bold uppercase tracking-widest text-[#64748B] mb-2 block">
            {language === 'hi' ? 'अपेक्षित MF रिटर्न' : 'Expected MF Return (%)'}
          </label>
          <input type="number" step={0.5} min={1} max={30} value={mfReturn} onChange={e => setMfReturn(Number(e.target.value))}
          ...
          <p className="text-[10px] text-[#64748B] mt-1">Nifty 50 historical avg: ~12% p.a.</p>
        </div>
        <div>
          <label className="text-xs font-bold uppercase tracking-widest text-[#64748B] mb-2 block">
            {language === 'hi' ? 'FD ब्याज दर' : 'FD Interest Rate (%)'}
          </label>
          <input type="number" step={0.25} min={1} max={15} value={fdRate} onChange={e => setFdRate(Number(e.target.value))}
            className="w-full bg-[#0A0F1E] border border-[#1E3A5F] rounded-xl py-3 px-4 text-white focus:border-[#1A56DB] outline-none font-mono" />
          <p className="text-[10px] text-[#64748B] mt-1">Top bank FDs: 7-8.5% p.a.</p>
        </div>
        <div>
          <label className="text-xs font-bold uppercase tracking-widest text-[#64748B] mb-2 block">
      ...
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.25 }}
        ...
        <p className="text-lg sm:text-xl font-bold font-syne" style={{ color: fdWins ? '#3B82F6' : '#F59E0B' }}>
          {fdWins ? '🛡️ FD wins by ' : '📈 Mutual Fund wins by '}
          {formatCurrency(diff)}
        </p>
        <p className="text-xs text-[#64748B] mt-1">
          {fdWins
            ? (language === 'hi' ? 'इस अवधि के लिए, गारंटीड FD रिटर्न टैक्स के बाद अपेक्षित MF रिटर्न से बेहतर है।' : 'For this horizon, the guaranteed FD return beats the expected MF return after tax.')
            : (language === 'hi' ? 'अगर बाज़ार अपेक्षित रूप से प्रदर्शन करें, तो MF FD से बेहतर टैक्स-पश्चात रिटर्न देता है।' : 'If markets perform as expected, MF gives better post-tax returns than FD.')}
        </p>
      </motion.div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-10">
        ...
        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.3 }}
          ...
          <div className="flex items-center gap-2 mb-1">
            <h2 className="text-xl font-syne font-bold text-white">{language === 'hi' ? 'फिक्स्ड डिपॉज़िट' : 'Fixed Deposit'}</h2>
          </div>
          ...
          <div className="space-y-3 text-sm">
            <Row label={language === 'hi' ? 'ग्रॉस दर' : 'Gross Rate'} value={`${actualFdRate.toFixed(2)}% p.a.`} />
            <Row label={language === 'hi' ? 'ग्रॉस मैच्योरिटी' : 'Gross Maturity'} value={formatCurrency(fdGross)} />
            <Row label={`TDS (10%)`} value={`-${formatCurrency(fdTDS)}`} red />
            <Row label={`Tax (${taxSlab}%)`} value={`-${formatCurrency(fdTotalTax)}`} red />
          </div>
          <div className="mt-5 bg-[#112240] rounded-xl p-5 border border-[#1E3A5F]">
            <div className="text-[10px] font-bold uppercase tracking-widest text-[#64748B] mb-1">{language === 'hi' ? 'नेट मैच्योरिटी' : 'NET MATURITY'}</div>
            <div className="text-3xl font-mono font-bold text-[#F59E0B]">{formatCurrency(fdNet)}</div>
            <div className="text-xs text-[#3B82F6] font-bold mt-1">Post-tax yield: {fdPostTaxYield.toFixed(2)}% p.a.</div>
          </div>
          <div className="mt-4 flex items-center gap-2 text-xs text-[#10B981]">
            <ShieldCheck size={14} /> DICGC Insured (up to ₹5L)
          </div>
          <div className="text-xs text-[#64748B] mt-1">{language === 'hi' ? 'जोखिम: शून्य — बैंक द्वारा गारंटीड' : 'Risk: Zero — Guaranteed by bank'}</div>
        </motion.div>

        {/* MF Card */}
        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.4 }}
          className="bg-[#0A0F1E] border-2 border-[#F59E0B]/40 rounded-2xl p-6 relative overflow-hidden">
          <div className="absolute top-0 right-0 p-4 opacity-5"><TrendingUp size={100} className="text-[#F59E0B]" /></div>
          <div className="flex items-center gap-2 mb-1">
            <h2 className="text-xl font-syne font-bold text-white">{language === 'hi' ? 'म्यूचुअल फंड (डायरेक्ट)' : 'Mutual Fund (Direct)'}</h2>
          </div>
          ...
          <div className="space-y-3 text-sm">
            <Row label={language === 'hi' ? 'अपेक्षित रिटर्न' : 'Expected Return'} value={`${mfReturn}% p.a.`} />
            <Row label={language === 'hi' ? 'ग्रॉस मैच्योरिटी' : 'Gross Maturity'} value={formatCurrency(mfGross)} />
            <Row label={`LTCG Tax (12.5%)`} value={`-${formatCurrency(mfTax)}`} red note={language === 'hi' ? '₹1.25L से ऊपर लाभ पर' : 'On gains above ₹1.25L'} />
          </div>
          <div className="mt-5 bg-[#F59E0B]/10 rounded-xl p-5 border border-[#F59E0B]/20">
            <div className="text-[10px] font-bold uppercase tracking-widest text-[#F59E0B] mb-1">{language === 'hi' ? 'नेट मैच्योरिटी' : 'NET MATURITY'}</div>
            <div className="text-3xl font-mono font-bold text-[#F59E0B]">{formatCurrency(mfNet)}</div>
            <div className="text-xs text-[#F59E0B] font-bold mt-1">Post-tax return: {mfPostTaxReturn.toFixed(2)}% p.a.</div>
          </div>
          <div className="mt-4 flex items-center gap-2 text-xs text-red-400">
            <AlertTriangle size={14} /> {language === 'hi' ? 'DICGC बीमा नहीं — बाज़ार जोखिम' : 'Not DICGC insured — Market risk applies'}
          </div>
          <div className="text-xs text-[#64748B] mt-1">{language === 'hi' ? 'जोखिम: मध्यम से उच्च' : 'Risk: Moderate to High'}</div>
        </motion.div>
      </div>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }}
        ...
        <h3 className="text-lg font-syne font-bold text-white mb-4">
          {language === 'hi' ? 'वर्ष-दर-वर्ष मूल्य वृद्धि' : 'Year-by-Year Value Growth'}
        </h3>
        <ResponsiveContainer width="100%" height={280}>
          <BarChart data={chartData} barGap={4}>
            <XAxis dataKey="year" stroke="#64748B" fontSize={11} tickLine={false} axisLine={false} />
            <YAxis tickFormatter={(v: number) => `₹${(v / 100000).toFixed(0)}L`} stroke="#64748B" fontSize={10} tickLine={false} axisLine={false} />
            <Tooltip
              formatter={(v: number, name: string) => [`₹${Number(v).toLocaleString('en-IN')}`, name]}
              contentStyle={{ background: '#112240', border: '1px solid #1E3A5F', borderRadius: '8px', fontSize: '11px' }}
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }}
        ...
        {insights.map((ins, i) => (
          <div key={i} className="bg-[#0D1A2E] rounded-xl p-5 border border-[#1E3A5F] hover:border-[#1A56DB]/40 transition-colors">
...
function Row({ label, value, red, note }: { label: string; value: string; red?: boolean; note?: string }) {
  return (
    <div className={`flex justify-between items-center border-b border-[#1E3A5F]/50 pb-2 ${red ? 'text-red-400' : ''}`}>
      <span className="text-[#94A3B8]">
        {label}
        {note && <span className="block text-[10px] text-[#64748B]">{note}</span>}
      </span>
