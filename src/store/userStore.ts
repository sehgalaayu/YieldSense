...
interface UserStore {
  ...
  bookFD: (
    fd: FDProduct,
    amount: number,
    booking?: { maturityDate?: string; maturityAmount?: number },
  ) => void;
...
export const useUserStore = create<UserStore>()(
  persist(
    (set) => ({
        ...
        set((state) => ({
          bookedFDs: [
            ...state.bookedFDs,
            {
              ...
              maturityDate: booking?.maturityDate,
      ...
      mfHoldings: [],
      mfAnalysisResults: [],
      addMFHolding: (holding) =>
        set((state) => ({ mfHoldings: [...state.mfHoldings, holding] })),
      removeMFHolding: (fundId) =>
        set((state) => ({
          mfHoldings: state.mfHoldings.filter((h) => h.fundId !== fundId),
        })),
      setMFAnalysisResults: (results) => set({ mfAnalysisResults: results }),
      clearMFAnalysis: () => set({ mfAnalysisResults: [], mfHoldings: [] }),

      navCache: {},
      setNavCache: (schemeCode, nav) =>
        set((state) => ({
          navCache: { ...state.navCache, [schemeCode]: nav },
        })),
