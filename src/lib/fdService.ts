...
export const getFDRates = async (): Promise<FDProduct[]> => {
  ...
  try {
    ...
    fdCache = data.map((row) => ({
      ...
      specialFeatures: row.special_features ? [row.special_features] : [],
    ...
    case "Safest":
      return allFDs
        ...
        .sort((a, b) => {
          ...
          return (
            (safetyOrder[b.bankType as keyof typeof safetyOrder] || 0) -
            (safetyOrder[a.bankType as keyof typeof safetyOrder] || 0)
          );
        });
